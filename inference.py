...
from kospeech.core import load_audio
...
def result():
    ...
    feature = parse_audio('./chunk1.wav', del_silence=True)
    input_length = torch.LongTensor([len(feature)])
    vocab = KsponSpeechVocabulary('./aihub_labels.csv')

    model = torch.load('./model.pt', map_location=lambda storage, loc: storage).to(opt.device)
